...
![镜像下载位置](../98-Notebook-Images/EVE-NG环境搭建/01-镜像下载位置.png)
**2. Windows 工具包（可选但建议）**：页面下方的`Windows Client Side`是一个集成了一些必要客户端工具（如 PuTTY、Wireshark 等）的包，建议下载以方便后续操作
![Win工具包下载](../98-Notebook-Images/EVE-NG环境搭建/01-Win工具包下载.png)
...
![EVE登陆界面](../98-Notebook-Images/EVE-NG环境搭建/01-EVE登陆界面.png)
...
![Web界面登录](../98-Notebook-Images/EVE-NG环境搭建/01-Web界面登录.png)

**2. 创建实验(Lab)**：点击`Add New Lab`创建一个测试Lab，选中Lab后点击`Open`进入Lab
![创建及打开Lab](../98-Notebook-Images/EVE-NG环境搭建/01-创建及打开Lab.png)

**3. 添加并启动设备**：
点击左上角`Add an object`，选择`Node`，然后在`Template`中选择刚刚导入的设备模板，然后下面的设备参数可以先不调整，点击`Save`保存
![添加节点](../98-Notebook-Images/EVE-NG环境搭建/01-添加节点.png)
然后在设备图标**右键**-`Start`即可令设备开机
![设备开机](../98-Notebook-Images/EVE-NG环境搭建/01-设备开机.png)
...
![通过PuTTY查看ip和端口](../98-Notebook-Images/EVE-NG环境搭建/01-通过PuTTY查看ip和端口.png)
...
![Lab中添加网络](../98-Notebook-Images/EVE-NG环境搭建/01-EVE-Lab中添加网络.png)

**2. 连接设备**：接下来在设备关机的情况下，拖动设备图标右侧的`connect to another node`图标，连接到我们添加的**Cloud0**网络上，并在弹出窗口中选择要互联的物理接口

![设备连接到网络](../98-Notebook-Images/EVE-NG环境搭建/01-设备连接到网络.png)

**3. 配置与测试**：
通过Telnet连接到设备的控制台，为互联端口配置一个与**真机同网段**的IP地址和掩码，然后Ping真机IP地址查看连通性

![虚拟CE ping真机](../98-Notebook-Images/EVE-NG环境搭建/01-虚拟CE-ping真机.png)

然后用真机Ping设备的IP地址查看连通性

![真机ping虚拟CE](../98-Notebook-Images/EVE-NG环境搭建/01-真机ping虚拟CE.png)
