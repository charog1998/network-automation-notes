...
```
`input, output, choice, case, grouping, uses`这些节点被称为模式节点，它们在**XPath**中是需要跳过的。因此在`path ../../../payment-methods/payment-method/method;`中，第一次`..`是从`method`跳转到了`payment`容器，第二次`..`是从`payment`跳转到了`purchase`动作，第三次`..`是从`purchase`跳转到了`user`（当前用户），接下来一路向下到`user/payment-methods/payment-method/method`

接下来，在文件开头处添加`revision`信息：
```yang
revision 2025-12-17 { 
  description 
...
module: bookzone-example
  ...
  +--rw users
     +--rw user* [user-id]
        ...
        |  +--ro format                   -> /books/book[title=current()/../title]/format/format-id        
        ...
        +---x purchase
           ...
           |  +---w format?             -> /books/book[title=current()/../title]/format/format-id  
           |  +---w number-of-copies?   uint32
           |  +---w payment
           |     +---w method?   -> ../../../payment-methods/payment-method/method
           |     +---w id?       -> ../../../payment-methods/payment-method[method=current()/../method]/id
           +--ro output
              +--ro (outcome)?
                 +--:(success)
                 |  +--ro success?        empty
